...
export function useReaderBridge(callbacks: ReaderBridgeCallbacks) {
  ...
  const pendingChapterTTSSegmentsResolveRef = useRef(
    new Map<string, (segments: VisibleTTSSegment[]) => void>(),
  );
  ...
  const getChapterTTSSegments = useCallback(
    (startHref: string, endHref: string | null | undefined, count = 24) => {
      return new Promise<VisibleTTSSegment[]>((resolve) => {
        const requestId = createRequestId("chapter-tts");
        pendingChapterTTSSegmentsResolveRef.current.set(requestId, resolve);

        webViewRef.current?.injectJavaScript(`
        (function() {
          try {
            if (window.doGetChapterTTSSegments) {
              window.doGetChapterTTSSegments(${JSON.stringify(startHref)}, ${JSON.stringify(endHref || null)}, ${Math.max(1, count)}, ${JSON.stringify(requestId)});
            } else {
              window.ReactNativeWebView.postMessage(JSON.stringify({type:'chapterTTSSegments',requestId:${JSON.stringify(requestId)},segments:[],error:'doGetChapterTTSSegments not defined'}));
            }
          } catch(e) {
            window.ReactNativeWebView.postMessage(JSON.stringify({type:'chapterTTSSegments',requestId:${JSON.stringify(requestId)},segments:[],error:String(e)}));
          }
        })();
        true;
      `);
        ...
        setTimeout(() => {
          const pendingResolve = pendingChapterTTSSegmentsResolveRef.current.get(requestId);
          if (pendingResolve === resolve) {
            pendingChapterTTSSegmentsResolveRef.current.delete(requestId);
            resolve([]);
          }
        }, 8000);
      });
    ...
    try {
      ...
      switch (msg.type) {
        ...
        case "hrefTTSSegments":
          ...
          break;
        case "chapterTTSSegments":
          {
            const requestId = typeof msg.requestId === "string" ? msg.requestId : null;
            const pendingResolve = requestId
              ? pendingChapterTTSSegmentsResolveRef.current.get(requestId)
              : pendingChapterTTSSegmentsResolveRef.current.values().next().value;
            if (pendingResolve) {
              if (msg.error) {
                console.warn("[ReaderBridge] chapterTTSSegments error:", msg.error);
              }
              pendingResolve(msg.segments || []);
              if (requestId) {
                pendingChapterTTSSegmentsResolveRef.current.delete(requestId);
              } else {
                pendingChapterTTSSegmentsResolveRef.current.clear();
              }
            }
          }
          break;
