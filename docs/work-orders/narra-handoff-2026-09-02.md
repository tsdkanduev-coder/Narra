...
Сделано в коде (см. ledger): P0-1 (слот сцены без бэкенда; проверено вживую — картинка сцены встала в текст), P0-2 (промоушен приоритета слота), P0-4 (описание героя после аудита), P0-3 частично (резолв героя в озвучке сцены), P0-6 (промпт сцены: бюджет gpt-image, жанр, глава, отрывок вокруг якоря), P0-7 (память треда Narra, spoiler-free по умолчанию), P0-10 (персист: карантин битого снимка, flush в фоне), P0-3 (голоса бэкенд-героев через план, не голос нарратора), P0-8 (терминальные ошибки импорта без бесконечного повтора), P0-5 частично (фильтр псевдо-персонажей «Рассказчик/Автор»), каст читалки для импортных/встроенных книг (`reader-cast.ts`). Итог проверок: app-expo vitest 846/846, tsc 0; gateway node --test 703 pass / 16 skipped. Ветка не запушена: `git push -u origin narra/core-loop-polish` (или в форк) и PR в `mishanaer/Narra:main` — по решению пользователя.
...
11. Эвалы: матрица в `docs/narra-evals.md`, живой CLI `pnpm evals:narra` готов и прогнан по двум книгам (15 ✅ / 1 ❌ детерминированных, 5 ✅ с LLM-судьёй) — единственное падение: профили «Преступления и наказания» на staging пустые, нужен операторский пересинтез после деплоя P0-4; «Война и мир» 19/19. После деплоя ветки на staging повторить с `NARRA_EVALS_LLM=1` по двум книгам.
