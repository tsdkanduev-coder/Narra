...
### Живые эвалы · 2026-09-02 · `tools/narra-evals/run.mjs`

- CLI без зависимостей: регистрация тестовой установки → манифест/сцены/поиск/профили/текст/речь/чат с LLM-судьёй; отчёт JSON+MD; `pnpm evals:narra`.
- Прогон по «Преступлению и наказанию»: 10 ✅ / 3 ❌ (сцена на 10 % в очереди, поиск выключен на staging, 0/20 профилей с описанием) — см. `docs/narra-evals.md`. Чат героя: 3/3 у судьи. Речь: 15/16 голосов.

### P0-3 (продолжение) · каст читалки для импортных и встроенных книг · 2026-09-02

- Новый `src/lib/narra/reader-cast.ts`: `readerCastForCharacters` / `readerCastForBook` — бэкенд-герои при готовом непровизорном манифесте, иначе локальные/встроенные герои книги (раньше `ReaderScreen` отдавал в озвучку и подсветку имён только `backendManaged`, и у импортных книг каст был пустым — все реплики звучали нарратором, C4-RC2).
- `ReaderScreen.tsx`: каст берётся из селектора (deps: герои книги + манифест статуса бэкенда).
- Тесты: `reader-cast.test.ts` 6 кейсов; app-expo vitest 855/855, tsc 0. Не сделано: turn-taking диалогов в `voice-markup.ts` (агенты остановились по лимиту API).
