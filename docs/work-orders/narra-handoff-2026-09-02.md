...
11. Эвалы: матрица в `docs/narra-evals.md`, живой CLI `pnpm evals:narra` готов и прогнан (12 ✅ / 2 ❌ / 1 ⏭ — падения на стороне staging: очередь сцен без P0-2, профили без описаний до перезапуска синтеза после P0-4). После деплоя ветки на staging повторить с `NARRA_EVALS_LLM=1` по двум книгам.
