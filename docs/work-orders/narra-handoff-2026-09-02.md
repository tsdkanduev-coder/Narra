...
11. Эвалы: матрица в `docs/narra-evals.md`, живой CLI `pnpm evals:narra` готов и прогнан (10 ✅ / 3 ❌ — падения на стороне staging: очередь сцен, поиск выключен, профили без описаний). После деплоя ветки на staging повторить с `NARRA_EVALS_LLM=1` по двум книгам.
